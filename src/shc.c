...
static const char version[] = "Version 4.0.9-Hardened-Audit-2026";
...
static const char usage[] =
    "Usage: shc [--version] [-e DATE] [-m MESSAGE] [-i IOPT] [-x CMD] [-l "
    "LOPT] [-o OUTFILE] [-2ABCDhHpPrSUVv] -f SCRIPT";
...
static char *text;
static int verbose;
static const char SETUID_line[] = "#define SETUID %d	/* Define as 1 to call "
                                  "setuid(0) at start of script */\n";
...
static const char TRACEABLE_line[] = "#define TRACEABLE	%d	/* Define as 1 "
                                     "to enable ptrace the executable */\n";
static int TRACEABLE_flag = 1;
static const char HARDENING_line[] =
    "#define HARDENING	%d	/* Define as 1 to disable ptrace/dump the "
    "executable */\n";
static int HARDENING_flag = 0;
static const char MMAP2_line[] = "#define MMAP2		%d	/* Define as 1 "
                                 "to use syscall mmap2 */\n";
static int MMAP2_flag = 0;
static const char BUSYBOXON_line[] = "#define BUSYBOXON	%d	/* Define as 1 "
                                     "to enable work with busybox */\n";
...
static const char *RTC[] = {
    ...
    "#else", "#define _UNUSED_R(x) (void) (x)", "#endif", "#if HARDENING", "",
    ...
    "\"#define PLACEHOLDER \\\"********\\\"\",", "\"#include <dlfcn.h>\",",
    "\"#include <stdlib.h>\",", "\"#include <string.h>\",",
    "\"#include <unistd.h>\",", "\"#include <stdio.h>\",",
    "\"#include <signal.h>\",", "\"\",",
    "\"static char secret[128000]; //max size\",",
    "\"typedef int (*pfi)(int, char **, char **);\",",
    "\"static pfi real_main;\",", "\"\",", "\"// copy argv to new location\",",
    ...
    "#ifndef _DARWIN_C_SOURCE", "#define _DARWIN_C_SOURCE", "#endif",
    "#ifndef _POSIX_C_SOURCE", "#define _POSIX_C_SOURCE 200809L", "#endif", "",
    "#include <sys/stat.h>", "#include <sys/types.h>", "#include <sys/wait.h>",
    "", "#include <errno.h>", "#include <stdio.h>", "#include <stdlib.h>",
    "#include <string.h>", "#include <time.h>", "#include <unistd.h>",
    "#include <fcntl.h>", "#include <limits.h>", "", "#ifndef PATH_MAX",
    "#define PATH_MAX 4096", "#endif", "", "/* 'Alleged RC4' */", "",
    "static unsigned char stte[256], indx, jndx, kndx;", "", "/*",
    " * Reset arc4 stte.", " */", "void stte_0(void) {",
    "	indx = jndx = kndx = 0;", "	do {",
    "		stte[indx] = indx;", "	} while (++indx);", "}", "", "/*",
    ...
    "		stte[indx] = stte[jndx];", "		stte[jndx] = tmp;",
    "		tmp += stte[indx];", "		*ptr ^= stte[tmp];",
    "		ptr++;", "		len--;", "	}", "}", "",
    "/* End of ARC4 */", "", "#if HARDENING", "", "#include <sys/ptrace.h>",
    "#include <sys/wait.h>", "#include <signal.h>", "#if defined(__linux__)",
    "#include <sys/prctl.h>", "#define PR_SET_PTRACER 0x59616d61", "",
    "/* Seccomp Sandboxing Init */", "#include <stdlib.h>",
    "#include <stdio.h>", "#include <stddef.h>", "#include <string.h>",
    "#include <unistd.h>", "#include <errno.h>", "", "#include <sys/types.h>",
    "#include <sys/syscall.h>", "#include <sys/socket.h>", "",
    "#include <linux/filter.h>", "#include <linux/seccomp.h>",
    "#include <linux/audit.h>", "",
    "#define ArchField offsetof(struct seccomp_data, arch)", "",
    "#define Allow(syscall) \\",
    "    BPF_JUMP(BPF_JMP+BPF_JEQ+BPF_K, SYS_##syscall, 0, 1), \\",
    "    BPF_STMT(BPF_RET+BPF_K, SECCOMP_RET_ALLOW)", "",
    "struct sock_filter filter[] = {", "    /* validate arch */",
    ...
    "    Allow(brk),         /* for malloc(), inside libc */", "#if MMAP2",
    "    Allow(mmap2),       /* also for malloc() */", "#else",
    "    Allow(mmap),        /* also for malloc() */", "#endif",
    ...
    "        perror(\"Could not start seccomp:\");", "        exit(1);",
    "    }",
    "    if (prctl(PR_SET_SECCOMP, SECCOMP_MODE_FILTER, &filterprog) == -1) {",
    "        perror(\"Could not start seccomp:\");", "        exit(1);",
    "    }", "}", "#else", "void seccomp_hardening() {}", "#endif",
    "/* End Seccomp Sandboxing Init */", "",
    "void arc4_hardrun(void * str, int len) {", "	char *tmp2;",
    "	unsigned char tmp, * ptr;", "", "	if (len <= 0) { exit(1); }",
    "	tmp2 = malloc((size_t)len + 1U);", "	if (!tmp2) { exit(1); }",
    "	memcpy(tmp2, str, (size_t)len);", "	tmp2[len] = '\\0';",
    "	ptr = (unsigned char *)tmp2;", "", "	while (len > 0) {",
    "		indx++;", "		tmp = stte[indx];",
    ...
    "	key(control, sizeof(control));", "	return 0;", "}", "",
    "#if DEBUGEXEC", "void debugexec(char * sh11, int argc, char ** argv) {",
    "	int i;",
    ...
    "	string = getenv(buff);", "#if DEBUGEXEC",
    "	fprintf(stderr, \"getenv(%s)=%s\\n\", buff, string ? string : "
    "\"<null>\");",
    "#endif", "	l = strlen(buff);", "	if (!string) {",
    "		/* 1st */",
    "		snprintf(&buff[l], sizeof(buff)-l, \"=%lu %d\", mask, argc);",
    "		putenv(strdup(buff));", "		return 0;", "	}",
    ...
    "	if (!name || name_len == 0) { return; }", "	name[0] = '\\0';",
    "	snprintf(procfile, sizeof(procfile), \"/proc/%d/cmdline\", pid);",
    "	FILE* f = fopen(procfile, \"r\");", "	if (f) {",
    "		size_t size;",
    "		size = fread(name, sizeof(char), name_len - 1, f);",
    "		name[size] = '\\0';",
    "		if (size > 0 && ('\\n' == name[size - 1] || '\\0' == name[size "
    "- 1])) {",
    "			name[size - 1] = '\\0';", "		}",
    "		fclose(f);", "	}", "}", "", "void hardening() {",
    "#if defined(__linux__)", "	prctl(PR_SET_DUMPABLE, 0);",
    "	prctl(PR_SET_PTRACER, -1);", "", "	int pid = getppid();",
    "	char name[256] = {0};",
    ...
    "		if (!mine && errno != EBUSY) {",
    "			if ((mine=2*!ptrace(PT_ATTACHEXC, pid, 0, 0))) { "
    "wait(0); }",
    "		}", "		if (!mine) {",
    "			perror(argv0);",
    "			kill(pid, SIGKILL);",
    ...
    "*/",
    "#endif", "	} else {",
    "		varg[j++] = shll;		/* Interpreter path/name at "
    "execution */",
    "	}", "#endif", "	setenv(\"SHC_ARGV0\", argv[0], 1);",
    ...
    "tmpbase, (int)getpid());",
    "		if (n < 0 || (size_t)n >= sizeof(tdir) || !mkdtemp(tdir)) { "
    "exit(1); }",
    "		n = snprintf(tnm, sizeof(tnm), \"%s/script\", tdir);",
    "		if (n < 0 || (size_t)n >= sizeof(tnm) || mkfifo(tnm, "
    "S_IWUSR|S_IRUSR) != 0) {",
    "			rmdir(tdir);", "			exit(1);",
    "		}", "		if ((i=fork())) {",
    ...
    "				memset(text+i, 0, w);",
    "			}", "			close(fd);",
    "			unlink(tnm);", "			rmdir(tdir);",
    "			_exit(0);", "		}",
    "		if (!FIXARGV0) {", "			varg[j++] = tnm;",
    "			i0 = 1;", "			goto xec;",
    "		}", "	}", "	if (*opts) {",
    "		varg[j++] = opts;	/* Options on 1st line of code */",
    "	}", "	if (*inlo) {",
    "		varg[j++] = inlo;	/* Option introducing inline code */",
    "	}", "	char *cmd = NULL;", "	if (PIPESCRIPT && ret) {",
...
static int parse_an_arg(int argc, char *argv[]) {
  ...
  case 'f':
    if (file) {
      fprintf(stderr,
              "%s parse(-f '%s'): Can convert exactly one SCRIPT, but -f "
...
static const struct shell_entry {
  ...
  const char *pfmt;
    ...
    {"perl", "-e", "--", "exec('%s',@ARGV);",
     "$0='%s';open(F,'%s');@SHCS=<F>;close(F);eval(join('',@SHCS));"},
    ...
    {"ksh", "-c", "", "exec '%s' \"$@\"",
     ". %.0s'%s'"}, /* OK on Solaris, AIX and Linux (THX
                       <bryan.hogan@dstintl.com>) */
    {"tsh", "-c", "--", "exec '%s' \"$@\"", ". %.0s'%s'"}, /* AIX */
    {"ash", "-c", "--", "exec '%s' \"$@\"", ". %.0s'%s'"}, /* Linux */
    {"csh", "-c", "-b", "exec '%s' $argv:q",
     "source %.0s'%s'"}, /* AIX: No file for $0 */
...
static char *read_script(const char *filename) {
  ...
  if (!PIPESCRIPT_flag && arg_max > 0 && (double)l > 0.80 * (double)arg_max) {
    fprintf(stderr,
            "%s: WARNING!!\n"
            "   Scripts of length near to (or higher than) the current System "
            "limit on\n"
            "   \"maximum size of arguments to EXEC\", could comprise its "
            "binary execution.\n"
...
static void prnt_bytes(FILE *o, const char *ptr, int m, int l, int n) {
  ...
  for (i = 0; i < n; i++) {
    ...
    fprintf(o, "\\%03o",
            (unsigned char)((i >= m) && (i < l) ? ptr[i - m] : rand_chr()));
    if ((i & 0xf) == 0xf) {
      fprintf(o, "\"");
    }
  }
...
static int write_C(const char *filename, int argc, char * const argv[]) {
  ...
  if (!msg1 || !kwsh || !tst1 || !chk1 || !msg2 || !tst2 || !chk2 || !name) {
    fprintf(stderr,
            "%s: memory allocation failed while preparing generated C source\n",
            my_name);
  ...
  if (append_suffix(&msg1, mail) != 0) {
    fprintf(stderr,
            "%s: memory allocation failed while preparing expiration message\n",
            my_name);
    cleanup_write_c(msg1, msg2, chk1, chk2, tst1, tst2, kwsh, name);
    return -1;
  }
  ...
  if (append_suffix(&name, ".x.c") != 0) {
    fprintf(stderr,
            "%s: memory allocation failed while preparing output C filename\n",
            my_name);
    cleanup_write_c(msg1, msg2, chk1, chk2, tst1, tst2, kwsh, name);
    return -1;
  }
  ...
  fprintf(o, "\tAudit and Hardening by HARRY DS ALSYUNDAWY - ALSYUNDAWY IT "
             "SOLUTION (2026)\n\n\t");
  ...
  do {
    ...
    do {
      ...
      case 0:
        if (pswd_z >= 0) {
          prnt_array(o, pswd, "pswd", pswd_z, 0);
          pswd_z = done = -1;
          break;
        }
      /* fall through */
      case 1:
        if (msg1_z >= 0) {
          prnt_array(o, msg1, "msg1", msg1_z, 0);
          msg1_z = done = -1;
          break;
        }
      /* fall through */
      case 2:
        if (date_z >= 0) {
          prnt_array(o, date, "date", date_z, 0);
          date_z = done = -1;
          break;
        }
      /* fall through */
      case 3:
        if (shll_z >= 0) {
          prnt_array(o, shll, "shll", shll_z, 0);
          shll_z = done = -1;
          break;
        }
      /* fall through */
      case 4:
        if (inlo_z >= 0) {
          prnt_array(o, inlo, "inlo", inlo_z, 0);
          inlo_z = done = -1;
          break;
        }
      /* fall through */
      case 5:
        if (xecc_z >= 0) {
          prnt_array(o, xecc, "xecc", xecc_z, 0);
          xecc_z = done = -1;
          break;
        }
      /* fall through */
      case 6:
        if (lsto_z >= 0) {
          prnt_array(o, lsto, "lsto", lsto_z, 0);
          lsto_z = done = -1;
          break;
        }
      /* fall through */
      case 7:
        if (tst1_z >= 0) {
          prnt_array(o, tst1, "tst1", tst1_z, 0);
          tst1_z = done = -1;
          break;
        }
      /* fall through */
      case 8:
        if (chk1_z >= 0) {
          prnt_array(o, chk1, "chk1", chk1_z, 0);
          chk1_z = done = -1;
          break;
        }
      /* fall through */
      case 9:
        if (msg2_z >= 0) {
          prnt_array(o, msg2, "msg2", msg2_z, 0);
          msg2_z = done = -1;
          break;
        }
      /* fall through */
      case 10:
        if (rlax_z >= 0) {
          prnt_array(o, rlax, "rlax", rlax_z, 0);
          rlax_z = done = -1;
          break;
        }
      /* fall through */
      case 11:
        if (opts_z >= 0) {
          prnt_array(o, opts, "opts", opts_z, 0);
          opts_z = done = -1;
          break;
        }
      /* fall through */
      case 12:
        if (text_z >= 0) {
          prnt_array(o, text, "text", text_z, 0);
          text_z = done = -1;
          break;
        }
      /* fall through */
      case 13:
        if (tst2_z >= 0) {
          prnt_array(o, tst2, "tst2", tst2_z, 0);
          tst2_z = done = -1;
          break;
        }
      /* fall through */
      case 14:
        if (chk2_z >= 0) {
          prnt_array(o, chk2, "chk2", chk2_z, 0);
          chk2_z = done = -1;
          break;
        }
      /* fall through */
      case 15:
        if (pfmt_z >= 0) {
          prnt_array(o, pfmt, "pfmt", pfmt_z, 0);
          pfmt_z = done = -1;
          break;
        }
      }
...
static void do_all(int argc, char *argv[]) {
  ...
  if (strstr(shll, "python") || strstr(shll, "perl") || strstr(shll, "csh")) {
